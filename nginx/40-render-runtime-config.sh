...
envsubst '${API_URL} ${GOOGLE_CLIENT_ID} ${PAYOS_RETURN_URL} ${POSTHOG_PROJECT_TOKEN} ${POSTHOG_HOST} ${ENABLE_DIAGNOSIS_ADDONS} ${ENABLE_GITHUB_EVIDENCE} ${APP_VERSION} ${GIT_SHA} ${BUILD_TIME}' < "${template}" > "${output}"
