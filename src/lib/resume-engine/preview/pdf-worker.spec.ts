...
describe("withPdfWorkerCacheKey", () => {
	it("adds a stable cache key so browsers refetch the pdf.js worker after MIME fixes", () => {
		...
		expect(url).toBe("https://www.skillbridgebuilder.com/assets/pdf.worker.min-Dtn11Elq.mjs?v=mjs-mime-20260706");
	});
	...
	it("preserves existing worker query params", () => {
		...
		expect(url).toBe("https://www.skillbridgebuilder.com/assets/pdf.worker.min-Dtn11Elq.mjs?x=1&v=mjs-mime-20260706");
	});
});
