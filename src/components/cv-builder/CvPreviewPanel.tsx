    ...
    <div className="flex flex-col h-full w-full relative bg-[#f3f4f6]" ref={containerRef}>
      ...
      {!isEmpty && (
        <div className="absolute bottom-8 left-1/2 -translate-x-1/2 flex items-center gap-1 bg-[#18181b]/95 backdrop-blur-md rounded-full px-3 py-1.5 border border-zinc-700/50 shadow-2xl z-20">
          <Button variant="ghost" size="icon" className="w-8 h-8 rounded-full text-zinc-400 hover:bg-zinc-800 hover:text-zinc-100" onClick={() => setScale(s => Math.max(0.2, s - 0.1))} title={t("builder.previewZoomOut")}>
          ...
          <Button variant="ghost" size="icon" className="w-8 h-8 rounded-full text-zinc-400 hover:bg-zinc-800 hover:text-zinc-100" onClick={() => setScale(s => Math.min(2, s + 0.1))} title={t("builder.previewZoomIn")}>
            <ZoomIn className="w-4 h-4" />
          </Button>
          <div className="w-px h-4 bg-zinc-700 mx-1" />
          <Button variant="ghost" size="icon" className="w-8 h-8 rounded-full text-zinc-400 hover:bg-zinc-800 hover:text-zinc-100" onClick={handleFitWidth} title={t("builder.previewFitWidth")}>
            <ArrowLeftRight className="w-3.5 h-3.5" />
          </Button>
          <Button variant="ghost" size="icon" className="w-8 h-8 rounded-full text-zinc-400 hover:bg-zinc-800 hover:text-zinc-100" onClick={handleFitPage} title={t("builder.previewFitPage")}>
            <ArrowUpDown className="w-3.5 h-3.5" />
          </Button>
          <div className="w-px h-4 bg-zinc-700 mx-1" />
          <Button variant="ghost" size="icon" className="w-8 h-8 rounded-full text-zinc-400 hover:bg-zinc-800 hover:text-zinc-100" onClick={toggleFullscreen} title={isFullscreen ? t("builder.previewExitFullscreen") : t("builder.previewFullscreen")}>
