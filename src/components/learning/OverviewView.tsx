...
function getSessionsForDay(dayIndex: number, allSessions: LearningSession[]) {
  return allSessions.filter(s => s.dayOfWeek === calendarIndexToSessionDay(dayIndex));
}

function calendarIndexToSessionDay(dayIndex: number) {
  return dayIndex === 6 ? 0 : dayIndex + 1;
}

function sessionSubjectKey(session: LearningSession) {
  return session.skillCanonical ?? session.moduleId ?? session.skill;
}

function subjectSessionPosition(allSessions: LearningSession[], session: LearningSession) {
  const subjectSessions = allSessions
    .filter((item) => sessionSubjectKey(item) === sessionSubjectKey(session))
    .sort((a, b) => a.sessionNumber - b.sessionNumber || a.id.localeCompare(b.id));
  const index = subjectSessions.findIndex((item) => item.id === session.id);
  return {
    current: index >= 0 ? index + 1 : session.sessionNumber,
    total: Math.max(1, subjectSessions.length),
  };
}

function getSkillAccentClass(skill: string): string {
  const lower = skill.toLowerCase();
  if (lower.includes("react")) return "bg-cyan-500";
  if (lower.includes("typescript") || lower === "ts") return "bg-blue-600";
  if (lower.includes("javascript") || lower === "js") return "bg-amber-500";
  if (lower.includes("docker")) return "bg-sky-500";
  if (lower.includes("sql")) return "bg-violet-500";
  if (lower.includes("next")) return "bg-slate-900";
  if (lower.includes("node")) return "bg-emerald-650";
  if (lower.includes("css")) return "bg-pink-500";
  if (lower.includes("html")) return "bg-orange-500";
  if (lower.includes("git")) return "bg-red-500";
  return "bg-slate-400";
}
...
export function OverviewView() {
  ...
  const roadmapModules = useMemo(() => {
    ...
    return [...subjects.values()].map((subject) => {
      ...
      const completedSessions = subject.sessions.filter((session) => session.status === "completed").length;
      const hasInProgress = subject.sessions.some((session) => session.status === "in-progress");
      const allCompleted = totalSessions > 0 && completedSessions === totalSessions;
      const status: "completed" | "in-progress" | "locked" = allCompleted
        ? "completed"
        : hasInProgress
          ? "in-progress"
          : "locked";
  ...
  const visibleWeekNumber = weekOffset >= 0 ? weekOffset + 1 : null;
  const visibleWeekSessions = useMemo(() => {
    if (visibleWeekNumber == null) return [];
    return weeks.find((week) => week.weekNumber === visibleWeekNumber)?.sessions ?? [];
  }, [visibleWeekNumber, weeks]);
  ...
  const todayCalendarIndex = new Date().getDay() === 0 ? 6 : new Date().getDay() - 1;
  const todaySessionDay = calendarIndexToSessionDay(todayCalendarIndex);
  const todaySessions = weekOffset === 0
    ? visibleWeekSessions.filter((session) => session.dayOfWeek === todaySessionDay && session.status !== "locked")
    : [];
      ...
      <Card className="border border-slate-200/85 shadow-md shadow-slate-100/40 overflow-hidden rounded-2xl bg-white">
        ...
        <div className="flex items-center justify-between px-6 py-4 border-b border-slate-150 bg-slate-50/30">
          <div className="flex items-center gap-4">
            <h3 className="text-lg font-extrabold text-slate-800 tracking-tight min-w-[145px]">{monthLabel}</h3>
            <div className="flex items-center gap-0.5 bg-slate-100/85 border border-slate-200/30 rounded-xl p-1 shadow-sm">
              <button
                onClick={() => setWeekOffset(o => o - 1)}
                className="w-8 h-8 rounded-lg hover:bg-white flex items-center justify-center transition-all duration-200 text-slate-600 hover:text-slate-800 hover:shadow-sm"
              ...
              <button
                ...
                className={cn(
                  "text-xs font-bold px-3.5 py-1.5 rounded-lg transition-all duration-200",
                  weekOffset === 0
                    ? "text-primary bg-white shadow-sm font-extrabold"
                    : "text-slate-650 hover:bg-white hover:text-slate-800 hover:shadow-sm"
                )}
              >
          ...
          <Dialog open={isRescheduleOpen} onOpenChange={setIsRescheduleOpen}>
            <DialogTrigger asChild>
              <Button variant="outline" size="sm" onClick={handleOpenReschedule} className="h-9 text-xs font-bold gap-2 border-slate-250 hover:border-primary/30 text-slate-700 hover:text-primary hover:bg-primary/5 rounded-xl shadow-sm transition-all bg-white px-4">
                      ...
                      <div className="space-y-1.5">
                         ...
                         >
                           {dayLabels.map((label, idx) => (
                             <option key={idx} value={calendarIndexToSessionDay(idx)}>{label}</option>
                           ))}
                         </select>
                      </div>
        ...
        <div className="overflow-x-auto min-w-full custom-scrollbar pb-2">
          <div className="grid grid-cols-7 divide-x divide-slate-100 min-w-[900px] border-b border-slate-100">
            {dates.map((date, idx) => {
              const sessions = getSessionsForDay(idx, visibleWeekSessions);
              const today = isToday(date);
              const isPastDay = isPastCalendarDate(date);
              ...
              return (
                <div
                  ...
                  onDrop={() => {
                    ...
                    if (persistedRoadmapId) {
                      patchLearningRoadmapSchedule(
                        persistedRoadmapId,
                        toPersistedSchedule(weekPlans, draggedSessionId, targetDay),
                      ).catch(() => undefined);
                    }
                  ...
                  className={cn(
                    "min-h-[220px] p-3 transition-all duration-300 relative flex flex-col",
                    today 
                      ? "bg-gradient-to-b from-primary/[0.055] via-sky-50/70 to-white border-t-2 border-t-primary shadow-[inset_0_2px_0_rgba(14,165,233,0.12)]" 
                      : isPastDay
                        ? "hover:bg-slate-50/30"
                        : "hover:bg-slate-50/30",
                    isDragOver && "bg-primary/[0.04] border-2 border-dashed border-primary/25 rounded-xl scale-[0.98] z-10 shadow-[inset_0_2px_8px_rgba(59,130,246,0.02)]"
                  )}
                >
                  {/* Day header */}
                  <div className="text-center mb-3">
                    <p className={cn(
                      ...
                      today ? "text-primary/90" : isPastDay ? "text-slate-500" : "text-slate-450"
                    )}>
                      {dayLabels[idx]}
                    </p>
                    <div className="mt-1 flex justify-center">
                      <span className={cn(
                        "w-7.5 h-7.5 rounded-full flex items-center justify-center text-xs font-black transition-all duration-300",
                        today
                          ? "bg-gradient-to-br from-primary to-blue-600 text-white shadow-md shadow-primary/25 scale-105"
                  ...
                  {sessions.length > 0 ? (
                          ...
                          >
                            ...
                            <div className="mb-2 flex w-full items-start justify-between gap-2">
                              <span className={cn(
                                ...
                                {isCompleted ? <CheckCircle2 className="h-3.5 w-3.5" /> : subjectPosition.current}
                              </span>
                              <span className={cn(
                                ...
                                {isCompleted ? "Done" : isPastDay ? (i18n.language.startsWith("vi") ? "Đã qua" : "Past") : isActive ? "Ready" : "Locked"}
                              </span>
                            </div>
                            <p className={cn(
                            ...
                            <div className="mt-2 grid grid-cols-2 gap-1.5">
                              <span className="rounded-lg bg-slate-100 px-2 py-1 text-center text-[9px] font-black text-slate-500">
                                {i18n.language.startsWith("vi") ? "Buổi" : "Session"} {subjectPosition.current}/{subjectPosition.total}
                              </span>
                              <span className="inline-flex items-center justify-center gap-1 rounded-lg bg-slate-100 px-2 py-1 text-[9px] font-black text-slate-500">
                            ...
                            {/* Lock indicator */}
                            {isLocked && (
                              <div className="mt-auto inline-flex items-center justify-center gap-1 rounded-xl bg-white px-2 py-1.5 text-[9px] font-black text-slate-400 ring-1 ring-slate-200">
                                <Lock className="w-3 h-3 text-slate-400" /> {t("learning.status.locked")}
                              </div>
                            )}
                            {/* Stars & Duration */}
                    ...
                    <div className="h-full min-h-[90px] rounded-xl border border-dashed border-slate-200 flex flex-col items-center justify-center p-3 text-center bg-slate-50/40 group hover:bg-slate-50/70 hover:border-slate-300/80 transition-all duration-300">
                      <CalendarDays className="mb-2 h-5 w-5 text-slate-300" />
                      <p className="text-[10px] font-bold text-slate-400 tracking-wide uppercase">{t("learning.common.freeDay")}</p>
                      <p className="text-[8px] text-slate-400 mt-0.5">{i18n.language.startsWith("vi") ? "Nghi ngoi" : "Take a break"}</p>
                    </div>
                  )}
                </div>
              );
...
function toPersistedSchedule(
  weekPlans: ReturnType<typeof useRoadmapStore.getState>["weekPlans"],
  movedSessionId: string,
  targetDayOfWeek: number,
) {
  return weekPlans.flatMap((week) =>
    week.sessions.map((session) => ({
      id: session.id,
      week_number: week.weekNumber,
      session_index: session.sessionNumber,
      suggested_day_of_week:
        session.id === movedSessionId ? targetDayOfWeek : session.dayOfWeek,
    })),
  );
}
